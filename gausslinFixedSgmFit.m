function [t, ft, y, pgl, tbnum] =...
            gausslinFixedSgmFit(dt, enr, loi, lli, uli, hreg, sgm)
% Fit data with gaussian+poly1 function.
...
%         pgl(1) is number of signal events, pgl(3) is background
...
gsfit_init = @(p,t,llim,ulim) en*(p(1)*...
    exp(-0.5*((t-p(2))/sgm).^2)/(sgm*sqrt(2*pi))+...
    p(3)/(ulim-llim));
...
pgs_i(3) = 0.5*(sum(dt(xb1:xb2)) + sum(dt(xb3:xb4)));
pgs_i(1) = sum(ft) - pgs_i(3);
...
xh1 = floor((pgl(2)-hreg-enr(1))/en) + 1;
xh2 = ceil((pgl(2)+hreg-enr(1))/en) + 1;

% total
tbnum(1,1) = sum(dt(xh1:xh2));
% background
tbnum(2,1) = pgl(4)*(enr(xh2)-enr(xh1))+...
    (enr(xh2)^2-enr(xh1)^2)*(pgl(3)-pgl(4)*(ulim-llim))...
    /(ulim^2-llim^2);

% calculate number of events under (-4 -2), (2 4) in hreg units
xb1 = round((pgl(2)-4*hreg-enr(1))/en) + 1;
xb2 = round((pgl(2)-2*hreg-enr(1))/en) + 1;
xb3 = round((pgl(2)+2*hreg-enr(1))/en) + 1;
xb4 = round((pgl(2)+4*hreg-enr(1))/en) + 1;
